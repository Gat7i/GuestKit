...
export default async function SuggestionsPage() {
  ...
  // 2. Récupérer toutes les suggestions avec leurs catégories ET leurs images
  ...
  // 6. Fonction pour obtenir l'image principale d'une suggestion
  const getMainImage = (suggestion: any) => {
    if (!suggestion.images || suggestion.images.length === 0) {
      return null
    }
    const mainImage = suggestion.images.find((img: any) => img.is_principal)
    return mainImage || suggestion.images[0]
  }
                ...
                <div key={categoryName} className="mb-10">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {typedItems.map((suggestion: any) => {
                      const mainImage = getMainImage(suggestion)
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="relative h-48 bg-gray-100 overflow-hidden">
                            {mainImage ? (
                              ...
                              <div className={`w-full h-full bg-gradient-to-br ${style.color} flex items-center justify-center`}>
                                <span className="text-6xl text-white/50">{style.icon}</span>
                              </div>
                            )}
                          ...
                          {/* Contenu */}
                          <div className="p-5">
                            <h4 className="text-lg font-bold text-gray-800 mb-2 group-hover:text-blue-600 transition line-clamp-1">
                              {suggestion.title}
                            </h4>
                        ...
                        >
                          ...
                          <div className="relative h-40 bg-gray-100 overflow-hidden">
                            {mainImage ? (
                              ...
                              <div className={`w-full h-full bg-gradient-to-br ${style.color} bg-opacity-90 flex items-center justify-center`}>
                                <span className="text-5xl text-white/50">{style.icon}</span>
                              </div>
                            )}
                            <div className="absolute inset-0 bg-gradient-to-t from-black/40 via-transparent to-transparent" />
                            <div className="absolute top-3 right-3">
                          ...
                          <div className="p-5">
                            <h4 className="text-lg font-bold text-gray-800 mb-2 group-hover:text-amber-600 transition line-clamp-1">
                              {suggestion.title}
                            </h4>
